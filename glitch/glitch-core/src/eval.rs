...
// ─── Fast evaluation path (raw pixel buffer, reusable stack) ────

#[inline(always)]
fn px_raw(data: &[u8], x: u32, y: u32, w: u32, h: u32) -> [u8; 4] {
    if x < w && y < h {
...
#[inline(always)]
fn fetch_boxed_raw(data: &[u8], w: u32, h: u32, x: i32, y: i32, r: u8, g: u8, b: u8) -> [Rgb; 9] {
    let mut boxed = [Rgb::default(); 9];
    let mut k = 0;
    for i in x - 1..=x + 1 {
        for j in y - 1..=y + 1 {
            if i == x && j == y {
                boxed[k] = Rgb { r, g, b };
            } else if i < 0 || j < 0 || i as u32 >= w || j as u32 >= h {
                boxed[k] = Rgb::default();
            } else {
                let idx = (j as usize * w as usize + i as usize) * 4;
                boxed[k] = Rgb {
                    r: data[idx],
                    g: data[idx + 1],
                    b: data[idx + 2],
...
/// Like [`eval`] but operates on a raw RGBA byte buffer and reuses a
/// caller-provided stack, eliminating per-pixel heap allocation and all
/// `DynamicImage` enum dispatch.
#[inline(always)]
pub fn eval_fast<R: RngCore>(
    ctx: EvalContext<'_>,
    pixels: &[u8],
    img_w: u32,
    img_h: u32,
    ...
    let div = |a: u8, b: u8| -> u8 { if b == 0 { a } else { a.wrapping_div(b) } };
    let modu = |a: u8, b: u8| -> u8 { if b == 0 { a } else { a.wrapping_rem(b) } };
    let bit_and_not = |a: u8, b: u8| -> u8 { a & !b };
    let weight = |a: u8, b: u8| -> u8 { (f64::from(a) * (f64::from(b) / 255.0)) as u8 };
    let three_rule = |v: u32, max: u32| -> u8 { (((255 * v) / max) & 255) as u8 };
    ...
    for tok in tokens.iter().cloned() {
        match tok {
            ...
            Token::Greater => {
                let vb = stack.pop().ok_or("Stack underflow")?;
                let va = stack.pop().ok_or("Stack underflow")?;
                stack.push(Rgb::new(
                    if va.r > vb.r { 255 } else { 0 },
                    if va.g > vb.g { 255 } else { 0 },
                    if va.b > vb.b { 255 } else { 0 },
                ));
            }

            Token::Random(num) => {
                let neg = std::ops::Neg::neg(num as i8);
                let cached = saved.v_r.as_ref().and_then(|v| v.get(&num));
                let val = if let Some(v) = cached {
                    *v
                } else {
                    let colors = gen_random_position(neg as i32, num as i32, rng);
                    let mut rgb = [0u8; 3];
                    for (i, (xx, yy)) in colors.iter().enumerate() {
                        let px = px_raw(pixels, (xx + x as i32) as u32, (yy + y as i32) as u32, img_w, img_h);
                        rgb[i] = px[i];
                    }
                    let c = Rgb::from(rgb);
                    if !ignore_state {
                        saved.v_r.get_or_insert_with(HashMap::new).insert(num, c);
                    }
                    c
                };
                stack.push(val);
            ...
            Token::Brightness(bv) => {
                let p = px_raw(pixels, x, y, img_w, img_h);
                let factor = (bv as f64 / 255.0).clamp(0.0, 1.0);
                let (nr, ng, nb) = adjust_brightness_hsv(p[0], p[1], p[2], factor);
                stack.push(Rgb::new(nr, ng, nb));
            }

            Token::Invert => {
                let p = px_raw(pixels, x, y, img_w, img_h);
                stack.push(Rgb::new(255 - p[0], 255 - p[1], 255 - p[2]));
            }
            ...
            Token::Char(c) => match c {
                ...
                'Y' => {
                    let v_y = match saved.v_y {
                        ...
                        None => {
                            let l = f64::from(b)
                                .mul_add(0.0722, f64::from(r).mul_add(0.299, f64::from(g) * 0.587));
                            let v = Rgb::new(l as u8, l as u8, l as u8);
                            saved.v_y = Some(v);
                            v
                        }
                    };
                ...
                's' => stack.push(Rgb::new(sr, sg, sb)),
                'x' => { let xu = three_rule(x, width); stack.push(Rgb::new(xu, xu, xu)); }
                'y' => { let yu = three_rule(y, height); stack.push(Rgb::new(yu, yu, yu)); }
                't' => {
                    let v_t = match saved.v_t {
                        Some(v) => v,
                        None => {
                            ...
                            let mut rgb = [0u8; 3];
                            for (i, (xx, yy)) in colors.iter().enumerate() {
                                let p = px_raw(pixels, (xx + x as i32) as u32, (yy + y as i32) as u32, img_w, img_h);
                                rgb[i] = p[i];
                            }
                            let v = Rgb::from(rgb);
                            if !ignore_state { saved.v_t = Some(v); }
                            v
                        }
                    };
                    stack.push(v_t);
                ...
                'g' => {
                    let v_g = match saved.v_g {
                        Some(v) => v,
                        None => {
                            let colors = gen_random_position(0, width as i32, rng);
                            let mut rgb = [0u8; 3];
                            for (i, (xx, yy)) in colors.iter().enumerate() {
                                let p = px_raw(pixels, (xx + x as i32) as u32, (yy + y as i32) as u32, img_w, img_h);
                                rgb[i] = p[i];
                            ...
                            let v = Rgb::from(rgb);
                            if !ignore_state { saved.v_g = Some(v); }
                            v
                        }
                    };
                    stack.push(v_g);
                ...
                'e' => {
                    let v_e = match saved.v_e {
                        Some(v) => v,
                        None => {
                            let bx = fetch_boxed_raw(pixels, img_w, img_h, x as i32, y as i32, r, g, b);
                            let v = Rgb::new(
                                bx[8].r.wrapping_sub(bx[0].r).wrapping_add(bx[5].r).wrapping_sub(bx[3].r)
                                    .wrapping_add(bx[7].r).wrapping_sub(bx[1].r).wrapping_add(bx[6].r).wrapping_sub(bx[2].r),
                                bx[8].g.wrapping_sub(bx[0].g).wrapping_add(bx[5].g).wrapping_sub(bx[3].g)
                                    .wrapping_add(bx[7].g).wrapping_sub(bx[1].g).wrapping_add(bx[6].g).wrapping_sub(bx[2].g),
                                bx[8].b.wrapping_sub(bx[0].b).wrapping_add(bx[5].b).wrapping_sub(bx[3].b)
                                    .wrapping_add(bx[7].b).wrapping_sub(bx[1].b).wrapping_add(bx[6].b).wrapping_sub(bx[2].b),
                            );
                            saved.v_e = Some(v);
                            v
                        }
                ...
                'b' => {
                    let v_b = match saved.v_b {
                        ...
                        None => {
                            let bx = fetch_boxed_raw(pixels, img_w, img_h, x as i32, y as i32, r, g, b);
                            let rr = wrapping_vec_add_u32([bx[0].r,bx[1].r,bx[2].r,bx[3].r,bx[5].r,bx[6].r,bx[7].r,bx[8].r]);
                            let gg = wrapping_vec_add_u32([bx[0].g,bx[1].g,bx[2].g,bx[3].g,bx[5].g,bx[6].g,bx[7].g,bx[8].g]);
                            let bb = wrapping_vec_add_u32([bx[0].b,bx[1].b,bx[2].b,bx[3].b,bx[5].b,bx[6].b,bx[7].b,bx[8].b]);
                            let v = Rgb::new((rr / 9) as u8, (gg / 9) as u8, (bb / 9) as u8);
                            if !ignore_state { saved.v_b = Some(v); }
                            v
                        }
                    };
                    stack.push(v_b);
                ...
                'H' => {
                    let v_h = match saved.v_high {
                        ...
                        None => {
                            let bx = fetch_boxed_raw(pixels, img_w, img_h, x as i32, y as i32, r, g, b);
                            let v = Rgb::new(
                                max([bx[0].r,bx[1].r,bx[2].r,bx[3].r,bx[5].r,bx[6].r,bx[7].r,bx[8].r]),
                                max([bx[0].g,bx[1].g,bx[2].g,bx[3].g,bx[5].g,bx[6].g,bx[7].g,bx[8].g]),
                                max([bx[0].b,bx[1].b,bx[2].b,bx[3].b,bx[5].b,bx[6].b,bx[7].b,bx[8].b]),
                            );
                            if !ignore_state { saved.v_high = Some(v); }
                            v
                        }
                    };
                    stack.push(v_h);
                ...
                'L' => {
                    let v_l = match saved.v_low {
                        ...
                        None => {
                            let bx = fetch_boxed_raw(pixels, img_w, img_h, x as i32, y as i32, r, g, b);
                            let v = Rgb::new(
                                min([bx[0].r,bx[1].r,bx[2].r,bx[3].r,bx[5].r,bx[6].r,bx[7].r,bx[8].r]),
                                min([bx[0].g,bx[1].g,bx[2].g,bx[3].g,bx[5].g,bx[6].g,bx[7].g,bx[8].g]),
                                min([bx[0].b,bx[1].b,bx[2].b,bx[3].b,bx[5].b,bx[6].b,bx[7].b,bx[8].b]),
                            );
                            if !ignore_state { saved.v_low = Some(v); }
                            v
                        }
                    };
                    stack.push(v_l);
                ...
                'h' => {
                    let v_h = match saved.v_h {
                        Some(v) => v,
                        None => {
                            let p = px_raw(pixels, width.wrapping_sub(x).wrapping_sub(1), y, img_w, img_h);
                            let v = Rgb::new(p[0], p[1], p[2]);
                            if !ignore_state { saved.v_h = Some(v); }
                            v
                        }
                    };
                    stack.push(v_h);
                ...
                'v' => {
                    let v_v = match saved.v_v {
                        Some(v) => v,
                        None => {
                            let p = px_raw(pixels, x, height.wrapping_sub(y).wrapping_sub(1), img_w, img_h);
                            let v = Rgb::new(p[0], p[1], p[2]);
                            if !ignore_state { saved.v_v = Some(v); }
                            v
                        }
                    };
                    stack.push(v_v);
                ...
                'd' => {
                    let v_d = match saved.v_d {
                        Some(v) => v,
                        None => {
                            let dx = width.wrapping_sub(x).wrapping_sub(1);
                            let dy = height.wrapping_sub(y).wrapping_sub(1);
                            let p = px_raw(pixels, dx, dy, img_w, img_h);
                            let v = Rgb::new(p[0], p[1], p[2]);
                            if !ignore_state { saved.v_d = Some(v); }
                            v
                        }
                    };
                    stack.push(v_d);
